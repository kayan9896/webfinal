...
Xiaoyang Zhao kayan9896&test2\    server part
...
Here is the home page of the client.
...
When we click the game, we can see the game information page.
...
At the end of the game information, we allow log-in users to add, edit and delete their comments.
...
And we also have a search bar, a mini game page with the detail page fetching data from our database. Xiaoyang Zhao contributed to these functions.
...
Meixuan completed the log in and user profile.
...
Xiaoyang Zhao set up the server, chieved the CRUD functions mongodb and connected the server with React.
...
Arghavan Ayoughi handled the data from steam api and user comments for the server.
...
Finally, we deployed the server and the client separately on render. Xiaoyang Zhao is responsible for the deployment.
